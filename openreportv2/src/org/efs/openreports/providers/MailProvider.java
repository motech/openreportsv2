...
public class MailProvider {
	protected static Logger log = Logger
			.getLogger(MailProvider.class.getName());
	...
	public MailProvider(PropertiesProvider propertiesProvider)
	...
	protected void init() throws ProviderException {
		...
		if (property != null)
			useMailAuthenticator = Boolean.valueOf(property.getValue())
					.booleanValue();

		property = propertiesProvider
				.getProperty(ORProperty.MAIL_SMTP_ENABLE_TLS);
		if (property != null)
			enableTls = Boolean.valueOf(property.getValue()).booleanValue();
		...
		try {
			if (mailSession == null) {
				...
				if (mail.getBounceAddress() != null
						&& mail.getBounceAddress().trim().length() > 0) {
					mailProps.setProperty("mail.smtp.from",
							mail.getBounceAddress());
				}
				if (mail.getBounceAddress() != null
						&& mail.getBounceAddress().trim().length() > 0) {
					mailProps.setProperty("mail.smtp.from",
			...
			if (mail.getText() != null
					&& (mail.getByteArrayDataSource() == null || !mail
							.getByteArrayDataSource().getContentType()
			...
			for (int i = 0; i < recipients.size(); i++) {
				...
				msg.addRecipient(recipientType,
						new InternetAddress(tokenizer.nextToken()));
			}
