	...
	public String execute() {
		try {
			if (submitType == null) {
				...
				reportGenerationDirectory = directoryProvider
						.getReportGenerationDirectory();

				ORProperty property = propertiesProvider
						.getProperty(ORProperty.MAIL_SMTP_HOST);
				if (property != null)
					mailHost = property.getValue();

				property = propertiesProvider
						.getProperty(ORProperty.MAIL_SMTP_PORT);
				if (property != null)
					mailPort = property.getValue();

				property = propertiesProvider
						.getProperty(ORProperty.MAIL_SMTP_AUTH);
				if (property != null)
					mailAuthenticatorUsed = new Boolean(property.getValue())
							.booleanValue();

				property = propertiesProvider
						.getProperty(ORProperty.MAIL_SMTP_ENABLE_TLS);
				if (property != null)
					mailTlsUsed = new Boolean(property.getValue())
							.booleanValue();
				...
				propertiesProvider.setProperty(ORProperty.MAIL_SMTP_ENABLE_TLS,
						String.valueOf(mailTlsUsed));
				mailProvider.setEnableTls(mailTlsUsed);

				propertiesProvider.setProperty(ORProperty.MAIL_SMTP_HOST,
						mailHost);
				if (mailHost != null)
					mailProvider.setMailHost(mailHost);
				if (mailHost != null)
					mailProvider.setMailHost(mailHost);
				propertiesProvider.setProperty(ORProperty.MAIL_SMTP_PORT,
						mailPort);
				if (mailHost != null)
					mailProvider.setMailPort(mailPort);
				property = propertiesProvider
						.getProperty(ORProperty.MAIL_AUTH_USER);
				if (property != null)
					mailUser = property.getValue();

				property = propertiesProvider
						.getProperty(ORProperty.MAIL_AUTH_PASSWORD);
				if (property != null)
				...
				property = propertiesProvider
						.getProperty(ORProperty.XMLA_CATALOG);
				if (property != null)
					xmlaCatalog = property.getValue();

				property = propertiesProvider
						.getProperty(ORProperty.XMLA_DATASOURCE);
				if (property != null)
					xmlaDataSource = property.getValue();

				property = propertiesProvider.getProperty(ORProperty.XMLA_URL);
				if (property != null)
					xmlaUri = property.getValue();
			...
			propertiesProvider.setProperty(ORProperty.BASE_DIRECTORY,
					baseDirectory);
			if (baseDirectory != null)
				directoryProvider.setReportDirectory(baseDirectory);

			propertiesProvider.setProperty(ORProperty.TEMP_DIRECTORY,
					tempDirectory);
			if (tempDirectory != null)
				directoryProvider.setTempDirectory(tempDirectory);

			propertiesProvider.setProperty(
					ORProperty.REPORT_GENERATION_DIRECTORY,
					reportGenerationDirectory);
			if (tempDirectory != null)
				directoryProvider
						.setReportGenerationDirectory(reportGenerationDirectory);

			propertiesProvider.setProperty(ORProperty.DATE_FORMAT, dateFormat);
			if (dateFormat != null)
				dateProvider.setDateFormat(dateFormat);

			propertiesProvider.setProperty(ORProperty.MAIL_AUTH_PASSWORD,
					mailPassword);
			...
			propertiesProvider.setProperty(ORProperty.MAIL_SMTP_AUTH,
					String.valueOf(mailAuthenticatorUsed));
			...
			propertiesProvider.setProperty(ORProperty.QUERYREPORT_MAXROWS,
					maxRows);
			propertiesProvider
					.setProperty(ORProperty.XMLA_CATALOG, xmlaCatalog);
			propertiesProvider.setProperty(ORProperty.XMLA_DATASOURCE,
					xmlaDataSource);
