...
public class JRQueryExecuter {
	...
	/**
      *
      */
	public static PreparedStatement getStatement(JRQuery query, Map parameters,
			Map values, Connection conn) throws JRException {
		PreparedStatement pstmt = null;

		if (conn != null) {
			JRQueryExecuter queryExecuter = new JRQueryExecuter(query,
					parameters, values);
			...
			if (chunks != null && chunks.length > 0) {
				...
				for (int i = 0; i < chunks.length; i++) {
					...
					case JRQueryChunk.TYPE_PARAMETER_CLAUSE: {
						...
						Object parameterValue = parameterValues
								.get(parameterName);
				...
				if (parameterNames != null && parameterNames.size() > 0) {
					...
					for (int i = 0; i < parameterNames.size(); i++) {
						...
						parameter = (JRParameter) parametersMap
								.get(parameterName);
						if (parameter == null) {
							throw new JRException("Parameter '" + parameterName
									+ "' found in query not defined on report.");
						}

						clazz = getBaseType(parameter.getValueClass(),
								parameterName);
						// FIXMEparameterValue = jrParameter.getValue();
						parameterValue = parameterValues.get(parameterName);
						parameterValue = getSingleValue(parameterValue,
								parameterName);
		...
		if (value.getClass().isArray()) {
			...
			default:
				throw new IllegalStateException("more than one ("
						+ array.length + ") value for " + description);
			}
		} else {
			return value;
		}
